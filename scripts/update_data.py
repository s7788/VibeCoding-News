...
TRUMP_QUERY = "Trump remarks OR Trump says OR Trump interview OR Trump Truth Social OR Trump tariff OR Trump speech when:30d"
TRUMP_STATEMENT_KEYWORDS = (
    "truth social",
    "remark",
    "remarks",
    "said",
...
def fetch_trump_news():
    try:
        items = fetch_google_news(TRUMP_QUERY, max_items=24)
        filtered = []
        for item in items:
            haystack = f"{item.get('title', '')} {item.get('summary', '')}".lower()
            if "trump" not in haystack:
                continue
            if not any(keyword in haystack for keyword in TRUMP_STATEMENT_KEYWORDS):
                continue
            filtered.append(item)
        filtered.sort(key=lambda item: parse_published_timestamp(item.get("published")), reverse=True)
        items = filtered[:12]
